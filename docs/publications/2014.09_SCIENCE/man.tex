...
\textit{positional bias} \cite{lerman2014leveraging}.  While this 
result applies mainly to tasks of a 
particular design, it shows that basic psychological phenomena
can profoundly influence crowdsourced data.  If human computation is to be
understood as a kind of computing architecture, it will certainly be 
necessary to account for the computational ramifications of these 
psychological phenomena at play in the HPU.
...
unlike CPUs and GPUs, HPUs are subject to \textit{hysteresis}, meaning
that their outputs depend on the history of their inputs.  Because 
task-repetition is inherent in their design, virtually all microtask studies,
and most crowdsourced studies are at risk from the form of bias we 
investigate here. We call the effects that earlier tasks exert on later ones 
...
exposures.  To this end, we define the extent of bias, $\theta$, induced by 
alternative exposures, to 
be the resulting change in the probability with which workers provide given 
responses (\# this is a general divergence metric between probability 
...
the \textit{extent} of framing-induced bias we found was insignificant 
...
rules out a seemingly-simple mechanism in which workers emphasize
content that had been present in earlier tasks.  Seeing content in prior tasks
...
same words are often repeated, there will of course be a smaller 
...
bias that we observed leads us to question 
whether that is the best approach.  Although the bias would be random in 
