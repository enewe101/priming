...
in the humanities and computer science, in research into medical imaging
technologies, and in many applications where expert opinion would normally be 
sought.  Here we show that an inherent
...
tasks influences the vocabulary that a worker uses when labeling 
subsequent images in profound an nuanced ways.  Prior tasks not only 
alter what a worker focuses on, 
but also modulate the richness and specialization of her vocabulary.  
While they can be a source of systematic bias, our
results suggest that intertask effects can be leveraged
...
are subject to \textit{hysteresis}, meaning that their output depends
on the history of their inputs.  Given the
increasing prevalence of microtask-based methodologies, other factors 
...
Framing effects were comparable to intertask effects only when we had the
workers actively reiterate the frame by means of a combo-box input.
...
\begin{figure}
	...
	\caption{
		...
		materials is shown in the supplementary material.
	}
...
objects, (treatment \textit{task1:obj}), and then performed test tasks, in 
which they 
...
in \textit{exp2}.  We again compared
intertask effects (in treatment pair \textit{task2}) to framing effects 
...
In all treatments, workers were shown images one at a time, and provided
5 labels for each.  From the perspective of the worker, there was no
...
of task performance, there is a crucial difference in the question at issue. 
...
$\mathcal{D}$ unspecified.  Since worker's responses may
vary, repeating the same task might lead to different outcomes for 
$\mathcal{D}$.  We shall define the notion of \textit{bias} in terms of the
effect that prior tasks have on the probability of a positive outcome
from $\mathcal{D}$.

The nature of $\mathcal{D}$ will affect our measurement of bias.
If $\mathcal{D}$ has random output, or if it always outputs ``yes'' 
...
show that this
problem can be formulated as a  machine learning problem, wherein one
constructs a classifier that infers the exposure of a worker from her 
...
For each treatment pair, we trained a naive Bayes classifier (\# in the 
supplementary material we discuss the choice of classifier, the
approach to measuring accuracy, and draw a comparison to results using 
a support vector machine in place of the naive Bayes classifier) to 
distinguish workers from one treatment
(e.g \textit{task1:food}) from those of the conjugate treatment 
(e.g. \textit{task1:obj}).  In \textit{task1} and \textit{task2}, intertask 
effects profoundly biased worker outputs, whereas we could not detect 
a statistically significant bias induced by framing in \textit{frame1} and 
\textit{frame2} (see Fig.~\ref{fig:theta}).  The bias induced
by framing was only of a comparable level to that induced by intertask 
effects in \textit{echo1}.  
Moreover, asking workers to reiterate the
frame, as was done in \textit{echo1}, is probably interpreted as
a direct signal from the requester that the frame should be acted upon.  In that sense, 
\textit{echo1} is perhaps more indicative of the effects of an 
\textit{instruction}.  This shows that
intertask effects profoundly affect the responses of workers, producing an
...
To observe the dynamics of intertask effects, we trained a naive Bayes
...
task shows the greatest bias, after which the effects drop off, but 
...
which washes out the effects of initial tasks.
...
less than that in \textit{task2:cult}. Also, the effects on the
number of food references was, overall, somewhat smaller than one might 
have expected given the extent of bias observed for \textit{task1}, 
\textit{echo1}, and \textit{task2}.  This means that task and frame exposures
induce other effects which are more subtle than a bulk shift in number 
...
Negative priming occurs when repeated exposure to a stimulus that is 
perceived to be non-salient begins to be ignored (\#).  
...
fact that an image contains food does not appear to be salient, since it 
does not serve to distinguish one image from another.  Thus, the most 
generic reference to the fact that food appears, namely the label ``food'', 
is suppressed, as by negative priming, while more specialized
...
recommendations for the consideration of those in the position of designing 
any microtask, citizen science, or crowdsourcing initiative:
\begin{enumerate}
	\item{
		Require active feedback to emphasize important details, 
		as we did in \textit{echo1}.  Similarly, use training tasks rather
		than (only) examples.
	}
	\item{
		Although ``clean'' examples may be useful to provide clarity,
		training tasks should contain the same kinds of noise or otherwise
	...
	\item{
		...
		These should be dually considered as \textit{calibration tasks}.  
	...
	\item{
		...
		most traces are not negative (\#).  In the absence of positive 
		examples, the target concept may degrade or drift.
		Drawing validation/calibration tasks from the underrepresented class,
		while providing feedback might help sustain the target concept.
	}
	\item{
		For more precise classifications, it may help to begin with
		an initial coarse classification.  For example, in detecting 
		inappropriate comments, workers might initially choose from  
		\texttt{okay}, \texttt{inappropriate}, and \texttt{borderline}.  
		Following the coarse classification, the 
		\texttt{borderline} examples could be batched together.  Workers could
		then calibrate to finer distinctions while coding a more homogeneous 
		set of images.
	}
\end{enumerate}
...
consideration.  Although we found that intertask effects can be a major 
source of bias, they might be used to drive tune workers' attention and 
specificity relative to
a target concept.  Microtask work, citizen science, and other crowdsourcing
initiatives, as well task requiring expert opinions, often share the common 
feature that they rely on repetitive human judgments.  Therefore, such 
applications all stand to benefit from the findings and suggestions 
presented here.
...
Unlike the naive Bayes classifier, it is necessary to tune the cost and 
