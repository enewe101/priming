...
computing architecture, so it to maintaining the view of the HPU computing 
...
origin\footnote{
	Though we attempted to choose foods with less specific or iconic cultural
	associations, the initial tasks 
...
\caption{
	...
	using a combo box input (see Fig.~S\ref{fig:hit_preamble}C).
...
instructions (see Fig. S\ref{fig:hit_preamble}B and C).  
Initial tasks, when included, followed next.  The initial tasks consisted of 
a series of 5 slides, each containing an image-labeling task like the one 
shown in 
...
suggested the purpose of the tasks.
...
This \textit{echo-food-objects} treatment certainly produced the 
strongest effect of the framing experiments, but it is difficult to say 
whether that should be considered indicative of framing textit{per se}.  
The explicitness of the statement would tend to make it more like an 
instruction in its effect.  Perhaps more importantly, requiring the worker
to re-iterate the framing statement by means of a combo-box emphasizes 
the framing to a greater degree than the instructions, which would probably
tend to give the impression that the framing statement is intended as an
instruction to be acted upon.  Framing or not, the fact that initial tasks
can produce an effect that is on par with that of \textit{echo-food-objects}
shows just how strong intertask effects are. 
...
applications, due to the finding that many corpera are not themselves
homogeneous \cite{kilgarriff1996comparing}.  
This results from the fact that corpera are made of many
...
corpera to be different according to a $\chi^2$ test might not 
...
(i.e. the number of distinct words occuring in an experiment).  
...
\subsection*{Data preprocessing}
	...
	inputs in the task interface, see Fig.~S\ref{fig:hit_preamble}) 
	...
	were misspelled, but only 3.2\% were mispelled after correction.
