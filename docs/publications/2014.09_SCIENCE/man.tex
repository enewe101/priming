...
specialization of their word choice. While these \textit{intertask effects} 
can be a 
...
coding and transcription, such as of voice recordings or handwritten notes,
...
retrieval systems\footnote{We provide a survey of task types in the supplementary text.}.
...
From the requester's perspective, microtask platforms resemble
a remote compute server, and some seriously consider microtask platforms 
as a new kind of computing architecture
...
academia\cite{wolfson2011look,Berinsky2012351},
leading researchers to investigate the factors affecting its reliability.  
...
Although HIT stands for ``Human Intelligence Task'', we use ``task'' to mean
the smallest repeatable unit of work.  For example, one HIT may involve 
labeling ten images with descriptive words (\textit{labels}), while for
the present discussion, we consider labeling a single image to be a task.
...
architecture, it will be necessary to consider that the computing elements 
...
summarizing texts, or providing feedback to a product review.
...
}.  Thus earlier tasks significant affect later ones, raising concerns 
about the the microtask methodology itself.
...
framing.  Bias ranges from 0 (meaning the exposures 
...
were on par with an explicit, actively reinforced statement of purpose.
...
experiment, the food-primed workers would label test tasks using 
...
were used significantly more specialized words, in reference to food 
