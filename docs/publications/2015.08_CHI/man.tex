...
Bundling tasks together reduces the amount of time spent choosing 
assignments and reduces the overhead of switching between working and 
choosing.  This may explain why the the pre-defined templates offered to 
help requesters set up their microtask jobs generally bundle many tasks 
together by default\footnote{mturk.com}.

The effects of task-repetition have been studied in the context of 
monitoring tasks by facility operators.  There the main concern is 
operator error due to boredom and distraction.  
However, in the microtask context, tasks are often ill-defined, and lack 
a clear notion of error.  For example for an image-labeling task
it would be difficult to decide which labels are `correct'.
Not only does this complicate the measurement of task-repitition effects, 
it suggests that their effect may be better understood as introducing 
\textit{bias} into responses, rather than \textit{error}.  

One of our present contributions is a method for determining
the extent of this bias.  Generally, there may be many possible responses 
to an ill-defined, qualitative microtask, so the responses actually
obtained can be viewed as being drawn from a distribution.
Design factors that affect how workers respond to a microtask will change 
this distribution.  Measuring the effect of such a design factor therefore
amounts to measuring how much the distribution of responses changes
There are good statistical techniques for detecting \textit{whether}
samples come from two different distributions, such as the $\chi^2$ test.  
But to properly weigh design decisions, a method is needed to measure
the \textit{extent} by which response distributions differ.
The direct substitution of sample frequencies in divergence 
calculations, although occaisionally seen in literature, has no statistical
basis and can easily lead to the ``detection'' of large changes when none 
exist.  We present a convenient and statistically sound method that can be 
applied even when the set of possible responses is very large 
(or infinite), such as for free text responses.  
While our purpose is to measure the 
effects of prior microtasks on later ones, this method can be used to 
calculate an empirical divergence of distributions from samples in any 
context.

[\#\#]
Unlike tasks that have a clear correct response, tasks that rely on 
worker's qualitative judgments may be particularly susceptible to 
\textit{priming effects}.
Priming is the phenomenon by which a person's response to a prompt is
influenced by their recent exposure to stimuli
...
task \cite{Gass1999549} %sohn2001task}
so the repetition of similar tasks creates many opportunities for 
strong priming effects. 
If tasks can act as primes for other tasks, it may help explain an earlier 
observation [*] that when interrupting one microtask with another,
the interruption is more disruptive when the tasks are \textit{similar} to 
one another.
